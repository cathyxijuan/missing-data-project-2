...
data1<-simuDatawithMiss[1:150,] #N=1000
...
E.inv.b <-  solve(t(deltab) %*% WmB%*% deltab) 
Ub <- (WmB- WmB %*% deltab %*% E.inv.b %*% t(deltab)%*%WmB) 
...
kb.obs <- sum(diag(WcmB.obs%*%WmBi%*%Ub%*%WmBi)) 
kb.obs.nonn <-sum(diag(Ub%*%WmBi%*%WcmB.obs%*%WmBi%*%Ub%*%GammaB)) 

kb.exp <- sum(diag(WcmB.exp%*%WmBi%*%Ub%*%WmBi)) 
kb.exp.nonn <-sum(diag(Ub%*%WmBi%*%WcmB.exp%*%WmBi%*%Ub%*%GammaB)) 
eigen(Wm)$values
eigen(WmBi)$values
eigen(WcmB.exp)$values
