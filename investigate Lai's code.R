...
data1 <-simuDatawithMiss[1:300,] #N=1000
N <- nrow(data1)
...
#fitA <-  lavaan:::lav_object_independence(fitAa, se=T) 
...
Omega <- solve(inspect(fit.sat, "h1.information.observed"))#he used inspect(fit.sat, "inverted.information.observed") but the order of the parameters in the matrix are switched around.. 
...
dxdm <- cbind(I.p, O.ppstar) #why identiy matrix?
...
S.inv <- chol2inv(chol(S)) #equivalent to solve(S.inv)!!!!omg why??
tx <- resid(fitA)$mean
?lavInspect

inspect(fitA, "fit")["fmin"]*2
...
Delta <- lavaan:::computeDelta(fitA@Model)[[1]] #TS #same as inspect(fitA, "delta")
dim(Delta)
a1 <- p+1
a2 <- p+pstar
mu.dev <- Delta[1:p,]
...
rmsea.adj <- sqrt(F.hat/M.df-c/(n*M.df)) #written in the format in our paper. 
rmsea.adj 
