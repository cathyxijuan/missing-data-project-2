...
complete.data <- HolzingerSwineford1939[, paste0("x", 1:9)]
...
#complete data AFIs using ML estimation
fit <- cfa(hypothesized.model,data=complete.data,estimator="ML") 

rmsea.ml<-lavInspect(fit,"fit")["rmsea"]  
cfi.ml<-lavInspect(fit,"fit")["cfi"] 
rmsea.ml  #0.09212148 
cfi.ml #0.9305597

#complete data AFIs using MLM estimation
fit<-
  cfa(hypothesized.model,data=complete.data,estimator="MLM") 

rmsea.mlm<-lavInspect(fit,"fit")["rmsea.robust"]  
cfi.mlm<-lavInspect(fit,"fit")["cfi.robust"] 
rmsea.mlm  #0.09112753 
cfi.mlm #0.93159
...
incomplete.data <- as.data.frame(lapply(complete.data, function(x) {
  ind <- as.logical(rbinom(nrow(complete.data), 1, missing.percentage))
  x[ ind ] <- NA
  x }))



#incomplete data AFIs using FIML estimation
fit<-
  cfa(hypothesized.model,data=incomplete.data,estimator="ML",missing="FIML") 
...
#####Computing different versions of FIML-C AFIs
#-------FIMLC RMSEA and CFI----#
...
#---substitute FIML parameter estimates into the complete.datalete data fit function; 
...
#--complete.dataute FIMLC RMSEA and CFI without small sample corrections (i.e.,V0)-#
...
#---complete.dataute small sample corrections for V1-V6-------#
...
#-complete.dataute FIMLC RMSEA and CFI with small sample corrections(i.e.,V1-V6)-# 
rmsea.fimlc.v1 <- sqrt(max((Fmin-k.fimlc.v1/n)/df, 0))
rmsea.fimlc.v2 <- sqrt(max((Fmin-k.fimlc.v2/n)/df, 0))
rmsea.fimlc.v3 <- sqrt(max((Fmin-k.fimlc.v3/n)/df, 0))
rmsea.fimlc.v4 <- sqrt(max((Fmin-k.fimlc.v4/n)/df, 0))
...
names(rmsea.fimlc) <- c("rmsea.fimlc.v0", 
                  "rmsea.fimlc.v1", "rmsea.fimlc.v2",
                  "rmsea.fimlc.v3", "rmsea.fimlc.v4", 
                  "rmsea.fimlc.v5", "rmsea.fimlc.v6")
cfi.fimlc <- c(cfi.fimlc.v0, 
         cfi.fimlc.v1, cfi.fimlc.v2,
         cfi.fimlc.v3, cfi.fimlc.v4,
         cfi.fimlc.v5, cfi.fimlc.v6)
names(cfi.fimlc) <- c("cfi.fimlc.v0",
                "cfi.fimlc.v1", "cfi.fimlc.v2", 
                "cfi.fimlc.v3", "cfi.fimlc.v4",
                "cfi.fimlc.v5", "cfi.fimlc.v6")
...
#####computing different versions of TS AFIs

#----------Fitting Hypothesized model-----------###

#-----Stage 1------#
fit1 <-cfa(hypothesized.model,data=incomplete.data,
...
n <- nrow(incomplete.data) #sample size
...
#------complete.dataute FIML RMSEA and CFI ------------#
rmsea.fiml<-lavInspect(fit1,"fit")["rmsea"]  
cfi.fiml<-lavInspect(fit1,"fit")["cfi"] 

#-----------TS RMSEA and CFI--------------#

#--complete.dataute TS RMSEA and CFI without small sample correction (i.e., V0) #
...
#----complete.dataute small sample corrections for V1-V2----#
...
#complete.dataute TS RMSEA and CFI with small sample corrections (i.e.,V1-V2)# 
...
cfi.ts <- c(cfi.ts.v0, cfi.ts.v1, cfi.ts.v2)

names(cfi.ts) <- c("cfi.ts.v0", 
                   "cfi.ts.v1","cfi.ts.v2" )
...
#comparing all AFIs
complete.data.AFIs <- c(rmsea.ml, rmsea.mlm, 
                        cfi.ml, cfi.mlm)
names(complete.data.AFIs) <- c("rmsea.ml", "rmsea.mlm", 
                               "cfi.ml", "cfi.mlm")

incomplete.data.AFIs <- c(rmsea.fiml, rmsea.fimlc,rmsea.ts,
                          cfi.fiml, cfi.fimlc,cfi.ts)

complete.data.AFIs
incomplete.data.AFIs 
